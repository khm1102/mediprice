# MediPrice — AGENTS.md
> Codex가 이 프로젝트에서 코드를 생성할 때 반드시 따라야 하는 규칙 문서.
...
GET    /api/hospitals/search               위치 + 다중 비급여항목 검색 (v2)
...
```
절대 추가 금지
  Co-Authored-By: Codex... 트레일러   (commit 메시지에 도구/AI 출처 표시 금지)
  🤖 Generated with [Codex]      (이모지 + 생성 표기 금지)
```
